...
public class Game extends Thread {
    ...
    public static final int[] dx = new int[]{-1, 0, 1, 0}, dy = new int[]{0, 1, 0, -1};
        ...
        for (int i = 0; i < this.inner_walls_count / 2; i++) {
            for (int j = 0; j < 1000; j++) {
                ...
                if (g[r][c] == 1 || g[this.rows - 1 - r][this.cols - 1 - c] == 1)
                    continue;
                if (r == this.rows - 2 && c == 1 || c == this.rows - 2 && r == 1)
                    continue;
                g[r][c] = g[this.cols - 1 - r][this.rows - 1 - c] = 1;
                break;
            }
        }
   ...
   private boolean nextStep() {
    ...
    private void sendMove() {   // 向俩个client传递移动信息
        lock.lock();
        try {
            JSONObject resp = new JSONObject();
            resp.put("event", "move");
    ...
    private void judge() {  // 判断俩名玩家下一步操作是否合法
