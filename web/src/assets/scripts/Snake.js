...
export class Snake extends GameObject {
    constructor(info, gamemap) {
        ...
        this.status = "idle"; // idle表示静止，move移动，die死亡

        this.dr = [-1, 0, 1, 0]; //上右下左
        this.dc = [0, 1, 0, -1];
        ...
        if (this.id === 1) this.eye_direction = 2; //右上角的蛇
        //蛇眼睛x的偏移量
        this.eye_dx = [
            [-1, 1],
            [1, 1],
    ...
    render() {
        ...
        for (let i = 1; i < this.cells.length; i++) {
            ...
            if (Math.abs(a.x - b.x) < this.eps && Math.abs(b.y - a.y) < this.eps)
                continue;
